...
typedef union{
	unsigned char Byte[8];	// acces in bufferul de date la nivel de octet
	unsigned int Word[4];	// acces in bufferul de date la nivel de cuvant
} CAN_Data;
...
void CAN_Init (void);				// Initializeaza C_CAN in regim de test (Silent + Loopback), la 1 Mbit/sec
int CAN_Init_Object(char MsgObjNum, char CANID_Type, unsigned long CANID, char ObjDLC, char MsgObjDir); // initializare obiect
void CAN_Transmit (char MsgObjNum, unsigned char NewID, CAN_Msg *ptr);		// transmisie mesaj din obiect
void CAN_Receive (char MsgObjNum, CAN_Msg *ptr);									// preluare mesaj receptionat in obiect 