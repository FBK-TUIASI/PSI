...
unsigned char UART0_GetByteHex(void){
	return 16 * UART0_GetDigitHex() + UART0_GetDigitHex();
}

unsigned char UART0_GetDigitHex(void){
	unsigned char ch;
...
void UART0_PutByteHex(unsigned char input) {
   char temp,digit,str[2];
	...
	for (digit=0; digit<2; digit++) UART0_Putch(str[digit]);
}
